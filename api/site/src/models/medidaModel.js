...
function buscarMedidasEmTempoReal(idColmeia, sensor) {
...
function buscarMedias(idColmeia, sensor) {
    ...
    if (process.env.AMBIENTE_PROCESSO == "producao") {
        ...
        instrucaoSql = `
        select avg(valorRegistrado) as valorRegistrado, DATE_FORMAT(dataHora,'%M %Y') as mes from registros join colmeia 
        on idColmeia = fkColmeia where fkSensor = ${sensor} and fkEmpresa = ${idColmeia} group by mes;`
    } else {
        console.log("\nO AMBIENTE (produção OU desenvolvimento) NÃO FOI DEFINIDO EM app.js\n");
        return
    }
